...
/*
 * init_args(t_pargs from user, set identifier flag, indicate data type, directions for user, must this be set?)
 * Mostly just makes place in memory for things we expect the user to set
 */
...
/*init_arg_def: does the same thing as the above function, except the above we use to create space for something we'll take from the user 
 * (e.g. in the above value is set to false or unspecified because we're waiting to read)
 * This function on the other hand sets the value when its initialized
 * */
...
/*
 *get_command_line_args(number of args passed, pointer to the array the reads command line args which are pointers of chars, number of args we expect, where the args are stored)
 * This function rips user input and stores it in the appropriate t_pargs structure, while countinh how many arguments we've found while shooting down bad ones [a great function for catching
...
int get_command_line_args(int argc, char * argv[], int n_args, t_pargs *params)
{
  int i, j;
  int n_arg_found = 0;
  for (i = 0; i < argc; ++i) // loops through number args passed
  {
    for (j = 0; j < n_args; ++j)
    {
      if (strcmp(argv[i],params[j].flag) == 0) // if we see something that matches a flag we're looking for
      {
        ++n_arg_found; // count that as an argument attempted to pass
        if (params[j].bSet) // but if that flag has been set already
        { /* Duplicate arg error */
          fprintf(stderr,"ERROR: you provided flag %s twice!\n",params[j].flag); // kill the program
          exit(3);
        }
        params[j].bSet = TRUE; // if not, its set now! 
        if (params[j].type != etBOOL) // Check if there's been a value set next to it if the flag is not for a bool type
        {
          if (i + 1 == argc)
          {
...
/*check_mand_args: is pretty self explanatory 
 * It checks if the array of args passed contains all of the necessary arguments for the function according to what we set up in the init 
 */
...
/*
 * build_filename: Builds a file for us to read from or write to based on user input. 
 * fnm will be used later in the read or write command, input name is usually the user input read from params, ft is the file type as indicated by an int specified in the gromacs top header
 * def_end is how we want to end the file, which helps sort out the type 
 */
void build_filename(tW_word fnm, tW_word ipt_name, int ft, const char *def_end)
{
  char *p;
  strcpy(fnm,ipt_name);
  if (ft == eTOP) // we expect a top file
  {
    p = strstr(ipt_name,".btp"); //p points to the place where .btp is first in the file 
    if (p == NULL) // if we dont ind a file ending in .btp
    {
  ...
  else if (ft == eTRAJ)
  {
    ...
    else if (strstr(ipt_name,".lmp")) { } //what fancy lammps compatible coding 
