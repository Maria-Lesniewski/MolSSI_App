...
*/
//pass the topology structure that we plan to store info in and the name of the file (pointer to a string we create with build filename usually 
...
// This is ugly...
  int nBondTypes = 10, nAngleTypes = 9, nDihedralTypes = 8, nBigDihedralTypes = 1, nIMNBTypes = 4, nOtherTypes = 59;
  ...
  const tW_word DihedralTypes[] = {"Proper Dih.:","Ryckaert-Bell.:","Restricted Dih.:","CBT Dih.:","Fourier Dih.:","Improper Dih.:","Improper Dih.:","Tab. Dih.:"};
  ...
  int ret_flag = 1;
  bool bFound = FALSE;
  int BIDX = 0, AIDX = 0, DIDX = 0, BDIDX = 0, IMNBIDX = 0;
  while (ret_flag == 1)
  {
    bFound = FALSE;
    ...
    else
    {
      ...
      if (! bFound) // MRD 01.24.2020
      {
        for (i = 0; i < nBigDihedralTypes; ++i)
        {
          if ((strstr(inp_line,BigDihedralTypes[i]) != NULL) && (! bFound))
          {
            get_next_line(fp,inp_line);// nr: X
            test_sscanf = sscanf(inp_line," nr: %d ",&inp_int);
            if (inp_int > 0)
            {
              get_next_line(fp,inp_line); // iatoms:
              mol->bigdih_nr += inp_int;
              int prev_n_bigdihs = mol->n_bigdihs;
              mol->n_bigdihs += inp_int/BIGDIH_DIV;
              int n_new_bigdihs = inp_int/BIGDIH_DIV;
              mol->bigdih_types = (int *) erealloc(mol->bigdih_types, mol->n_bigdihs * sizeof(int));
              mol->bigdih_type_top_cat = (int *) erealloc(mol->bigdih_type_top_cat, mol->n_bigdihs * sizeof(int));
              mol->bigdih_ijklm = (int **) erealloc(mol->bigdih_ijklm, mol->n_bigdihs * sizeof(int *));
              for (j = prev_n_bigdihs; j < mol->n_bigdihs; ++j) { mol->bigdih_ijklm[j] = (int *) ecalloc(5,sizeof(int)); }
              for (j = 0; j < n_new_bigdihs; ++j)
              {
                get_next_line(fp,inp_line);
                ...
                test_sscanf = sscanf(word_list[1],"type=%d",&inp_int);
                mol->bigdih_types[BDIDX] = inp_int;
                mol->bigdih_type_top_cat[BDIDX] = i;
                mol->bigdih_ijklm[BDIDX][0] = atoi(word_list[3]);
                mol->bigdih_ijklm[BDIDX][1] = atoi(word_list[4]);
                mol->bigdih_ijklm[BDIDX][2] = atoi(word_list[5]);
                mol->bigdih_ijklm[BDIDX][3] = atoi(word_list[6]);
                mol->bigdih_ijklm[BDIDX][4] = atoi(word_list[7]);
                ++BDIDX;
                efree(word_list);
              }
            }
...
bool read_tpr_dump(tW_word fnm, tW_gmx_topology *top)
{
  ...
  // MRD 01.17.2020
  top->molblocks = (tW_molblock *) ecalloc(n_molblocks, sizeof(tW_molblock));
  top->n_molblocks = n_molblocks;
/* MRD comment 01.17.2020
...
/*
 * MRD 01.17.2020 
 * Due to issue 11, we want to relax the requirement that n_molblocks = n_moltypes
 * we want to be able to have multiple molblocks of the same type
 * So now we need to get the number of moltypes a different way.
 * Option 1: go thru the file, count how many times you find a line that is
 * "   moltype (X):"
 * and that should give you the number of moltypes.
 * Option 2: each molblock has an index for its moltype. let's just find the highest
 * index and add 1 to get the number of different moltypes.
 * Option 2 seems easier
 */
  int n_molecule_types = 0; // MRD 01.2020
...
// MRD 01.17.2020
// New way to figure out number of moltypes
    if (inp_int + 1 > n_molecule_types) { n_molecule_types = inp_int + 1; }
//    strcpy(my_mols[i].molname,inp_word); comment MRD 01.17.2020
    ++LINE_IDX;
    test_sscanf = sscanf(header_lines[LINE_IDX]," #molecules = %d ",&inp_int);
//    my_mols[i].n_mols = inp_int; comment MRD 01.17.2020
// MRD 01.17.2020
    top->molblocks[i].n_mols = inp_int;
  }

// MRD 01.17.2020 we can do this now
  top->molecules = (tW_molecule *) ecalloc(n_molecule_types, sizeof(tW_molecule));
  tW_molecule *my_mols = top->molecules;
  top->contents->mols.nr = n_molecule_types;
  top->n_molecule_types = n_molecule_types; // No we don't NEED this in two places...
...
// MRD 01.17.2020 added error check...
    if (test_sscanf != 1)
    {
      fprintf(stderr,"ERROR: expected line to contain:  name=\"MolName\" \n"); 
      fprintf(stderr,"however, line is: %s",inp_line);
      exit(1);
    }
    elim_char(inp_word,'"');
// MRD 01.17.2020 we no longer have the molnames stored. so copy that now
    strcpy(my_mols[n_moltypes-1].molname,inp_word);

    get_moltype_info(fp,&(my_mols[n_moltypes-1]),&inp_line);

// MRD 01.17.2020 we don't want to do this here any more.
//    n_atoms += my_mols[n_moltypes-1].n_apm * my_mols[n_moltypes-1].n_mols;
  }
/* MRD 01.17.2020 we want to relax this requirement.
