...
int get_clean_line(FILE * fp_in, tW_line inp_line)
{
    ...
    strcpy(inp_line, ""); //filling our strings with empty 
    strcpy(before, "");
    strcpy(after, "");

    if (fgets(before, MAXLINELEN, fp_in) == NULL) { // Check if the file has anything in it, store what we read in before
	return -1;
    }
...
int remove_comments(char *before, char *after)
{
    ...
    l_inp = strcspn(before, COMMENTFLAG); //count how many characters before comment

    strncpy(after, before, l_inp); // save only the string up to the comment 

    after[l_inp] = '\0'; // put a null character at the end of after
...
// get_command_line_args(length_user_input, user_input_strings, length_args_we_expect, args_we_expect)
// This function takes the vector of arguments we've specified that we expect [stored in t_pargs]
// and checks how many of them are actually given by the user
// returns the number of arguments we found from the user
int get_command_line_args(int argc, char * argv[], int n_args, t_pargs *params)
{
  ...
  for (i = 0; i < argc; ++i) // For each arg the user passed
  {
    ...
    {
      if (strcmp(argv[i],params[j].flag) == 0) // If a flag matches
      {
        ++n_arg_found; // Hooray, increment the number of arguments we found
        if (params[j].bSet) // But check that we haven't found it already
        { /* Duplicate arg error */
          fprintf(stderr,"ERROR: you provided flag %s twice!\n",params[j].flag);
          exit(3);
        }
        params[j].bSet = TRUE; //If we haven't already found this arg and errored, let's note that we found it. 
        if (params[j].type != etBOOL) //If we weren't expecting just the flag (type bool arg)
        {
          if (i + 1 == argc) // Check if we are at the end of the passed command line string
          {
            fprintf(stderr,"ERROR: please provide a value after parameter flag: %s \n",params[j].flag);
            exit(2);
          }
          ++i; // if not take the next word after the flag and put it in the parameter value 
          strcpy(params[j].value,argv[i]);
        }
        else { strcpy(params[j].value,"True"); } //If we were just expecting a flag, then mark that we found this arg and keep going
        j = n_args; // Once we find the param the user passed we stop looking through the parameters we were looking to get and go to the next one
      }
    }
...
// print_arg_table() 
// This function prints the list of argumetns we expected to get with t_pargs and their current values 
...
/*check_mand_args()
 *Checks the status of the parameter list passed to see if a mandatory argument isn't set yet
...
void check_mand_args(t_pargs *params, int nPars)
{
  int i = 0;
  bool bKill = FALSE;
  for (i = 0; i < nPars; ++i) // loop through the number of args we wanted from the user
  {
    if ((params[i].bMandatory) && (! params[i].bSet)) // Check if it was required
    {
  ...
  if (bKill)
  {
    exit(EXIT_FAILURE); // Kill the program if we miss something
  }
}
/*build_filename(): Alters the passed filename (ipt_name) according to the 
 *                  passed filetype (ft) and the corresponding append name failsafe
 *		    stores the results in fnm
 *		    e.g. build_filename(fnm, "Jerry.txt", eTRAJ, ".btj")
 *		    will throw you a warning that Jerry doesn't have a proper traj ending and then append to Jerry.txt.btj
 */
